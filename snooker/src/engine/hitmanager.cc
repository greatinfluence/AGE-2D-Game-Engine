...
void Hitmanager::regist(Object& ih) {
	////gamelogger << "has something" << std::endl;
	lst.push_back(&ih);
}
...
static void moveforbounce(Object& oba, Object& obb, Vec normal) {
	...
	if(oba.isus()) {
		...
		Vec nvbhor = (vahor * ma + vbhor * mb + (vahor - vbhor) * ma * bounce) /
						(ma + mb);
		/*//gamelogger << "Type3!" << std::endl;
		//gamelogger << va.x << ' ' << va.y << std::endl;
		//gamelogger << vb.x << ' ' << vb.y << std::endl;
		//gamelogger << normal.x << ' ' << normal.y << std::endl;*/
		oba.setv(va - vahor + nvahor);
		obb.setv(vb - vbhor + nvbhor);
		/*//gamelogger << oba.getv().x << ' ' << oba.getv().y << std::endl;
		//gamelogger << obb.getv().x << ' ' << obb.getv().y << std::endl;*/
	}
}
...
void Hitmanager::detecthit() const {
	...
	for(auto i = lst.begin(); i != lst.end(); ++ i, ++ ps) {
		...
		for(; j != lst.end(); ++ j) {
			...
			//gamelogger << "In hit judgement" << std::endl;
			auto hitinfo = (**i).ishit(**j);
			//gamelogger << "Out hit judgement" << std::endl;
			if(hitinfo != nullptr) {
				//gamelogger << "In hits" << std::endl;
				(**i).accept(**j, *hitinfo);
				(**j).accept(**i, *hitinfo);
				//gamelogger << "Out hits" << std::endl;
				if(!(*i) -> ifview() && !(*j) -> ifview())
					moveforbounce(**i, **j, *hitinfo);
				//gamelogger << "Out movement" << std::endl;
			}
		}
	}
	}
